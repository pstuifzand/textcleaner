...
namespace Abacus
{
    public class Parser
    {
        ...
        public Parser(string s, int start = 0)
        {
            text = s;
            startIndex = start;
            index = startIndex;
        ...
        public ParseResult parse()
        {
            ...
            {
                while (!(Char.IsDigit(current()) || current() == '(' || current() == '.' || current() == '-') && index < text.Length)
                {
                    nextchar();
                }
                ...
                {
                    return new ParseResult(expr, index, index);
                }
                ...
                try
                {
                    ...
                    while (Char.IsWhiteSpace(text[index - 1]))
                    {
                        index--;
                    }

                    return new ParseResult(expr, startIndex, index);
                }
        ...
        public Expression parse_term()
        {
            ...
            while (current() == '*' || current() == '/' || current() == '%')
            {
                ...
                if (op == '/' && current() == '/') {
                    ...
                    num1 = new OperatorExpression("//", num1, num2);
                    skipws();
                } else {
                    skipws();
                    Expression num2 = parse_factor();
