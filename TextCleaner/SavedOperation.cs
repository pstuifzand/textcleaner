...
{
    internal class SavedOperation : Operation
    {
        private readonly Operation wrappedOperation;

        private readonly string[] savedArgs;

        public SavedOperation(Operation wrappedOperation, string[] args)
        {
            this.wrappedOperation = wrappedOperation;
            savedArgs = args;
        }

        public string Name => wrappedOperation.Name;

        public string[] ArgNames => wrappedOperation.ArgNames;

        public OperationResult Process(string text, string[] args)
        {
            // Call this operation with the saved args.
            return wrappedOperation.Process(text, savedArgs);
        }
    }
}
