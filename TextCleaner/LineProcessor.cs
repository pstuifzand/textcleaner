    ...
    public class LineProcessor : Processor
    {
        public OperationResult Process(string input, Operation op, string[] args)
        {
            ...
            foreach (var line in lines) {
                count++;
                var opResult = op.Process(line, args);
                if (!opResult.Keep) continue;

                foreach (var hl in opResult.InputHighlights) {
                    operationResult.InputHighlights.Add(new Highlight(count - 1, hl.Start, hl.End));
                }

                if (count == lines.Length) {
                    sb.Append(opResult.Text);
                } else {
                    sb.AppendLine(opResult.Text);
                }
            }

            operationResult.Text = sb.ToString();
            operationResult.Keep = true;
